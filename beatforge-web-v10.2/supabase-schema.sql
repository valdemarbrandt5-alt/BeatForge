...
-- Instrument charts. Existing rows remain full-mix charts.
...
alter table public.charts add constraint charts_instrument_check check (instrument in ('mix','vocals','drums','bass','melody'));
create index if not exists charts_youtube_instrument_idx on public.charts (youtube_url, instrument);
