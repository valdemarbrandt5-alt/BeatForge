-- Run after battle_royale_round_timing.sql.
-- Ready confirms the player immediately, but no round starts before the full
-- 15-second deadline. The existing battle_royale_tick RPC starts it at zero.
...
begin
  ...
  if m.id is null or m.status<>'loading' or now()<m.updated_at+interval '15 seconds' then return; end if;
