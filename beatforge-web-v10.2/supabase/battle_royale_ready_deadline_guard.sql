...
-- Fix the full 15 second deadline when each round enters the loading phase.
...
begin
  if new.status='loading' and old.status is distinct from 'loading' then
    ...
    and now()<coalesce(old.ready_deadline,old.updated_at+interval '15 seconds') then
    raise exception 'Ready countdown has not reached zero';
  end if;
  return new;
end $$;

drop trigger if exists battle_royale_ready_deadline_guard on public.battle_royale_matches;
create trigger battle_royale_ready_deadline_guard
before update on public.battle_royale_matches
for each row execute function public.battle_royale_guard_ready_deadline();

update public.battle_royale_matches
set ready_deadline=updated_at+interval '15 seconds'
where status='loading' and ready_deadline is null;

-- Keep Ready confirmation separate from the fixed start time.
...
begin
  ...
  if m.id is null or m.status<>'loading' or now()<coalesce(m.ready_deadline,m.updated_at+interval '15 seconds') then return; end if;
