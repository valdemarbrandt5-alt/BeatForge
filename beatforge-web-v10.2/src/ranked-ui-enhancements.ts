...
if (typeof window !== 'undefined') {
  ...
  const decorateRanks = () => {
    document.querySelectorAll('.rankedBackdrop.realRanked .rankedCard').forEach(node => {
      ...
      const cls = rankClass(card.textContent || '');
  ...
  const decorateLiveHud = () => {
    ...
    const ordered = [...players].sort((a,b) => scoreOf(b) - scoreOf(a));
    ordered.forEach((player, i) => { player.style.order = String(i); player.dataset.place = String(i + 1); });
  };
  ...
  const recoverCancelledLobby = async () => {
    ...
    try {
      ...
      if (!latest || latest.status !== 'cancelled' || Date.now() - new Date(latest.created_at).getTime() > 10 * 60 * 1000) return;
  ...
  const addStyles = () => {
    ...
    const style = document.createElement('style'); style.id = 'ranked-result-colors';
    style.textContent = `
      ...
      .rankedCard.rankBronze{--rankAccent:#cd7f32}.rankedCard.rankSilver{--rankAccent:#c8ced8}.rankedCard.rankGold{--rankAccent:#ffd43b}.rankedCard.rankPlatinum{--rankAccent:#57e0d1}.rankedCard.rankDiamond{--rankAccent:#6aa9ff}.rankedCard.rankMaster{--rankAccent:#c084fc}
      ...
      .rankedCard[class*="rank"] .rankedTier,.rankedCard[class*="rank"] .rankedTier b,.rankedCard[class*="rank"] .rankedTier strong{color:var(--rankAccent)!important}
      ...
      .game>.realRankedLiveHud.rankedScoreboard>small{display:none!important}
      .game>.realRankedLiveHud.rankedScoreboard>div{display:flex!important;flex-direction:column!important;gap:4px!important;width:100%!important;min-width:0!important}
      .game>.realRankedLiveHud.rankedScoreboard i{display:none!important}
      .game>.realRankedLiveHud.rankedScoreboard span:has(.rrMe,.rrOpp){position:relative!important;display:grid!important;grid-template-columns:minmax(0,1fr) auto!important;grid-template-rows:auto auto!important;align-items:center!important;width:100%!important;min-width:0!important;margin:0!important;padding:6px 7px 6px 25px!important;border-radius:8px!important;background:#151a24!important;border:1px solid #252c39!important;text-align:left!important;transform:none!important}
      .game>.realRankedLiveHud.rankedScoreboard span:has(.rrMe,.rrOpp):before{content:'#' attr(data-place);position:absolute;left:6px;top:50%;transform:translateY(-50%);font-size:10px;font-weight:1000;color:#8e95a5}
      .game>.realRankedLiveHud.rankedScoreboard span[data-place='1']{border-color:#ffd43b55!important}.game>.realRankedLiveHud.rankedScoreboard span[data-place='1']:before{color:#ffd43b}
  ...
  const start = () => {
    scan(); new MutationObserver(scan).observe(document.body, { childList:true, subtree:true, characterData:true });
    document.addEventListener('click', event => { const target = event.target as HTMLElement | null; if (target?.closest('.realRanked .syncLeave,.realRanked .rankedLeave,.realRanked .cancelRealMatch')) void leaveCurrentLobby(); }, true);
