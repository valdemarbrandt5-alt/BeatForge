...
if (typeof window !== 'undefined') {
  ...
  const captureResult = (result: Element) => {
    ...
    const perfect = numberFrom(text('.perfectStat b'));
    const great = numberFrom(text('.greatStat b'));
    const good = numberFrom(text('.goodStat b'));
    const miss = numberFrom(text('.missStat b'));
    ...
    const accuracyEl = meta[0]?.querySelector('b');
    if (accuracyEl) accuracyEl.textContent = `${accuracy.toFixed(1)}%`;
    cachedResult = {
      perfect: String(perfect), great: String(great), good: String(good), miss: String(miss),
