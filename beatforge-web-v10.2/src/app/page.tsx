...
type SavedChart={id:string,title:string,artist?:string|null,difficulty:Difficulty,lane_count:number,duration:number,notes:Note[],created_at:string,audio_path?:string|null};
...
export default function Home(){
 ...
 const saveChart=async()=>{if(!user||!supabase){setAuthOpen(true);return}if(!(await ensureProfile()))return;if(!saveTitle.trim()){setCloudMessage('Enter a song name.');return}if(!saveArtist.trim()){setCloudMessage('Enter an artist.');return}setCloudMessage('Saving chart…');let audioPath:string|null=null;if(fileRef.current){const ext=(fileRef.current.name.split('.').pop()||'audio').replace(/[^a-zA-Z0-9]/g,'');audioPath=`${user.id}/${crypto.randomUUID()}.${ext}`;const {error:audioError}=await supabase.storage.from('song-audio').upload(audioPath,fileRef.current,{contentType:fileRef.current.type||'audio/mpeg',upsert:false});if(audioError){setCloudMessage('Audio upload failed: '+audioError.message);return}}const cleanNotes=baseNotes.map(({id,time,lane,duration})=>({id,time,lane,...(duration?{duration}:{})}));const {error}=await supabase.from('charts').insert({user_id:user.id,title:saveTitle.trim(),artist:saveArtist.trim(),difficulty,lane_count:laneCount,duration:songDuration,notes:cleanNotes,audio_path:audioPath});if(error){if(audioPath)await supabase.storage.from('song-audio').remove([audioPath]);setCloudMessage(error.message);return}setCloudMessage(audioPath?'Chart + private audio saved ✓':'Chart saved ✓');setSaveOpen(false);setTimeout(()=>setCloudMessage(''),1800)};
 const loadMyCharts=async()=>{if(!user||!supabase){setAuthOpen(true);return}const {data,error}=await supabase.from('charts').select('id,title,artist,difficulty,lane_count,duration,notes,created_at,audio_path').eq('user_id',user.id).order('created_at',{ascending:false});if(error){setCloudMessage(error.message);return}setSavedCharts((data||[]) as SavedChart[]);setChartSearch('');setMyChartsOpen(true)};
