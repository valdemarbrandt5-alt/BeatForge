...
type SavedChart={id:string,title:string,artist?:string|null,difficulty:Difficulty,lane_count:number,duration:number,notes:Note[],created_at:string,user_id?:string,profiles?:{username?:string|null}|null};
...
export default function Home(){
 ...
 const loadDiscover=async()=>{if(!supabase)return;const {data,error}=await supabase.from('charts').select('id,title,artist,difficulty,lane_count,duration,notes,created_at,user_id,profiles(username)').order('created_at',{ascending:false}).limit(200);if(error){setCloudMessage(error.message);return}setDiscoverCharts((data||[]) as unknown as SavedChart[]);setDiscoverSearch('');setDiscoverOpen(true)};
 ...
 const beginSaveChart=()=>{if(!user){setAuthOpen(true);return}setSaveTitle(songName.replace(/\.(mp3|wav|ogg|m4a|aac|flac)$/i,''));setSaveArtist('');setCloudMessage('');setSaveOpen(true)};
 const checkAndSaveChart=async()=>{if(!supabase)return;const title=saveTitle.trim(),artist=saveArtist.trim();if(!title||!artist){setCloudMessage('Enter song name and artist.');return}const {data,error}=await supabase.from('charts').select('id,title,artist,difficulty,lane_count,duration,notes,created_at,user_id,profiles(username)').ilike('title',title).ilike('artist',artist).limit(20);if(error){setCloudMessage(error.message);return}if(data?.length){setDuplicateCharts(data as unknown as SavedChart[]);setSaveOpen(false);setDuplicateOpen(true);return}await saveChart()};
 const saveChart=async()=>{if(!user||!supabase){setAuthOpen(true);return}if(!(await ensureProfile()))return;if(!saveTitle.trim()){setCloudMessage('Enter a song name.');return}if(!saveArtist.trim()){setCloudMessage('Enter an artist.');return}setCloudMessage('Saving chart…');const cleanNotes=baseNotes.map(({id,time,lane,duration})=>({id,time,lane,...(duration?{duration}:{})}));const {error}=await supabase.from('charts').insert({user_id:user.id,title:saveTitle.trim(),artist:saveArtist.trim(),difficulty,lane_count:laneCount,duration:songDuration,notes:cleanNotes});if(error){setCloudMessage(error.message);return}setCloudMessage('Community chart saved ✓');setSaveOpen(false);setTimeout(()=>setCloudMessage(''),1800)};
 const loadMyCharts=async()=>{if(!user||!supabase){setAuthOpen(true);return}const {data,error}=await supabase.from('charts').select('id,title,artist,difficulty,lane_count,duration,notes,created_at,user_id').eq('user_id',user.id).order('created_at',{ascending:false});if(error){setCloudMessage(error.message);return}setSavedCharts((data||[]) as SavedChart[]);setChartSearch('');setMyChartsOpen(true)};
 ...
 const openSavedChart=async(chart:SavedChart)=>{const source=(chart.notes||[]).map((n,i)=>({...n,id:i,hit:false,miss:false,holding:false,completed:false}));if(audio.current){audio.current.pause();audio.current.currentTime=0}if(urlRef.current){URL.revokeObjectURL(urlRef.current);urlRef.current=null}setAudioUrl(null);fileRef.current=null;setSongName(chart.title);setDifficulty(chart.difficulty);setLaneCount(chart.lane_count);setDuration(chart.duration);setBaseNotes(source);setNotes(buildChart(source,chart.lane_count,chart.difficulty));setMyChartsOpen(false);setDiscoverOpen(false);setDuplicateOpen(false);setStatus('Community chart loaded · upload your copy of the song to hear music')};
 ...
 return <main><header><div><h1>BEAT<span>FORGE</span></h1><p>5 lane browser rhythm game · accounts enabled</p></div><div className="accountArea">{user?<><button className="accountBtn" onClick={loadDiscover}>DISCOVER</button><button className="accountBtn" onClick={loadMyCharts}>MY CHARTS</button><button className="accountBtn" onClick={()=>{setCloudMessage('');setProfileOpen(true)}}>{username||'SET USERNAME'}</button><span className="accountEmail">{user.email}</span><button className="accountBtn" onClick={logout}>LOG OUT</button></>:<button className="accountBtn" onClick={()=>{setAuthMode('login');setAuthMessage('');setAuthOpen(true)}}>LOGIN</button>}</div></header>
 ...
 <section className="controls"><span className="debug">{running?`${upcoming} upcoming notes`:`${notes.length} notes ready`}</span><button disabled={analyzing} onClick={running?stop:play}>{running?'STOP':analyzing?'ANALYZING…':'PLAY'}</button><button className="secondary" onClick={stop}>RESET</button><label className="volume"><span>VOLUME</span><input aria-label="Volume" type="range" min="0" max="1" step="0.01" value={volume} onChange={e=>{const v=Number(e.target.value);setVolume(v);if(audio.current)audio.current.volume=v}}/><b>{Math.round(volume*100)}%</b></label>{user&&songName!=='Demo chart'&&<button className="secondary" onClick={beginSaveChart}>SAVE CHART</button>}<div className="time">{time<0?`READY ${Math.ceil(-time)}`:fmt(time)} / {fmt(songDuration)}</div></section>
 <section className="settings"><h2>Keybinds</h2><p>Click a box, then press the key you want for that lane.</p><div className="binds">{keys.map((k,i)=><button key={i} onKeyDown={e=>{e.preventDefault();setKeys(a=>a.map((x,j)=>j===i?e.key.toLowerCase():x))}}>{i+1}<strong>{k.toUpperCase()}</strong></button>)}</div></section>{cloudMessage&&<div className="cloudToast">{cloudMessage}</div>}{profileOpen&&<div className="resultBackdrop" role="dialog" aria-modal="true"><div className="resultCard authCard"><small>BEATFORGE PROFILE</small><h2>Choose your username</h2><input className="authInput" maxLength={24} placeholder="Username" value={username} onChange={e=>setUsername(e.target.value.replace(/[^a-zA-Z0-9_]/g,''))} onKeyDown={e=>{if(e.key==='Enter')saveProfile()}}/><p className="authMessage">This name will appear on leaderboards.</p><div className="resultActions"><button onClick={saveProfile}>SAVE PROFILE</button><button className="secondary" onClick={()=>setProfileOpen(false)}>CLOSE</button></div></div></div>}{discoverOpen&&<div className="resultBackdrop" role="dialog" aria-modal="true"><div className="resultCard libraryCard"><small>BEATFORGE COMMUNITY</small><h2>Discover Charts</h2><div className="librarySearch"><span>⌕</span><input className="authInput" value={discoverSearch} onChange={e=>setDiscoverSearch(e.target.value)} placeholder="Search song, artist or creator…"/></div><div className="chartLibrary">{(()=>{const q=discoverSearch.trim().toLowerCase();const filtered=discoverCharts.filter(ch=>!q||ch.title.toLowerCase().includes(q)||(ch.artist||'').toLowerCase().includes(q)||(ch.profiles?.username||'').toLowerCase().includes(q));return filtered.length?filtered.map(ch=><button className="savedChart communityChart" key={ch.id} onClick={()=>openSavedChart(ch)}><div className="chartIdentity"><strong>{ch.title}</strong><em>{ch.artist||'Unknown artist'}</em></div><span>{ch.difficulty} · {ch.lane_count} lanes · by {ch.profiles?.username||'BeatForge user'}</span></button>):<p className="authMessage">No community charts match your search.</p>})()}</div><div className="resultActions"><button className="secondary" onClick={()=>setDiscoverOpen(false)}>CLOSE</button></div></div></div>}{duplicateOpen&&<div className="resultBackdrop" role="dialog" aria-modal="true"><div className="resultCard libraryCard"><small>SONG ALREADY EXISTS</small><h2>{saveArtist} · {saveTitle}</h2><p className="authMessage">BeatForge found {duplicateCharts.length} existing chart{duplicateCharts.length===1?'':'s'}. Use one with your own audio file, or create another chart anyway.</p><div className="chartLibrary">{duplicateCharts.map(ch=><button className="savedChart communityChart" key={ch.id} onClick={()=>openSavedChart(ch)}><strong>{ch.title}</strong><span>{ch.difficulty} · {ch.lane_count} lanes · by {ch.profiles?.username||'BeatForge user'}</span></button>)}</div><div className="resultActions"><button onClick={async()=>{setDuplicateOpen(false);await saveChart()}}>CREATE ANYWAY</button><button className="secondary" onClick={()=>setDuplicateOpen(false)}>CANCEL</button></div></div></div>}{saveOpen&&<div className="resultBackdrop" role="dialog" aria-modal="true"><div className="resultCard authCard"><small>SAVE TO LIBRARY</small><h2>Save Chart</h2><label className="fieldLabel">SONG NAME<input className="authInput" value={saveTitle} onChange={e=>setSaveTitle(e.target.value)} placeholder="Song name"/></label><label className="fieldLabel">ARTIST<input className="authInput" value={saveArtist} onChange={e=>setSaveArtist(e.target.value)} placeholder="Artist name" onKeyDown={e=>{if(e.key==='Enter')saveChart()}}/></label>{cloudMessage&&<p className="authMessage">{cloudMessage}</p>}<div className="resultActions"><button onClick={checkAndSaveChart}>SAVE CHART</button><button className="secondary" onClick={()=>setSaveOpen(false)}>CANCEL</button></div></div></div>}{myChartsOpen&&<div className="resultBackdrop" role="dialog" aria-modal="true"><div className="resultCard libraryCard"><small>BEATFORGE LIBRARY</small><h2>My Charts</h2><div className="librarySearch"><span>⌕</span><input className="authInput" value={chartSearch} onChange={e=>setChartSearch(e.target.value)} placeholder="Search song or artist…"/></div><div className="chartLibrary">{(()=>{const q=chartSearch.trim().toLowerCase();const filtered=savedCharts.filter(ch=>!q||ch.title.toLowerCase().includes(q)||(ch.artist||'').toLowerCase().includes(q));return filtered.length?filtered.map(ch=><div className="savedChartRow" key={ch.id}><button className="savedChart" onClick={()=>openSavedChart(ch)}><div className="chartIdentity"><strong>{ch.title}</strong><em>{ch.artist||'Unknown artist'}</em></div><span>{ch.difficulty} · {ch.lane_count} lanes · {ch.notes?.length||0} notes</span></button><button className="deleteChart" title="Delete chart" aria-label={`Delete ${ch.title}`} onClick={e=>deleteSavedChart(ch,e)}>DELETE</button></div>):<p className="authMessage">{savedCharts.length?'No charts match your search.':'No saved charts yet.'}</p>})()}</div><div className="resultActions"><button className="secondary" onClick={()=>setMyChartsOpen(false)}>CLOSE</button></div></div></div>}{authOpen&&<div className="resultBackdrop" role="dialog" aria-modal="true"><div className="resultCard authCard"><small>BEATFORGE ACCOUNT</small><h2>{authMode==='login'?'Welcome back':'Create account'}</h2><input className="authInput" type="email" placeholder="Email" value={authEmail} onChange={e=>setAuthEmail(e.target.value)}/><input className="authInput" type="password" placeholder="Password" value={authPassword} onChange={e=>setAuthPassword(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')submitAuth()}}/>{authMessage&&<p className="authMessage">{authMessage}</p>}<div className="resultActions"><button onClick={submitAuth}>{authMode==='login'?'LOGIN':'SIGN UP'}</button><button className="secondary" onClick={()=>{setAuthMode(m=>m==='login'?'signup':'login');setAuthMessage('')}}>{authMode==='login'?'CREATE ACCOUNT':'I HAVE AN ACCOUNT'}</button><button className="secondary" onClick={()=>setAuthOpen(false)}>CLOSE</button></div></div></div>}{resultsOpen&&<div className="resultBackdrop" role="dialog" aria-modal="true"><div className="resultCard"><small>SONG COMPLETE</small><h2>{songName}</h2><div className="finalScore">{score.toLocaleString()}</div><span className="scoreLabel">FINAL SCORE</span><div className="resultGrid"><div className="perfectStat"><b>{hitStats.perfect}</b><span>PERFECT</span></div><div className="goodStat"><b>{hitStats.good}</b><span>GOOD</span></div><div className="greatStat"><b>{hitStats.great}</b><span>GREAT</span></div><div className="missStat"><b>{hitStats.miss}</b><span>MISS</span></div></div><div className="resultMeta"><div><b>{accuracy.toFixed(1)}%</b><span>ACCURACY</span></div><div><b>{hitStats.maxCombo}×</b><span>MAX COMBO</span></div><div><b>{Math.abs(avgTimingMs).toFixed(0)} ms</b><span>{timingLabel}</span></div></div><div className="resultActions"><button onClick={()=>{setResultsOpen(false);play()}}>PLAY AGAIN</button><button className="secondary" onClick={()=>setResultsOpen(false)}>CLOSE</button></div></div></div>}<footer>BeatForge v0.22 · Community Charts</footer></main>
}
