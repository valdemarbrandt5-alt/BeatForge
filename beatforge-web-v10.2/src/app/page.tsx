...
export default function Home(){
 ...
 const play=async()=>{setResultsOpen(false);resetStats();setCombo(0);comboRef.current=0;setScore(0);setHoldScore(0);holdRemainder.current=0;lastFrameRef.current=null;setJudge('READY');setPressed(null);resetNotes();setTime(0);timeRef.current=0;if(audio.current&&audioUrl){audio.current.currentTime=0;try{await audio.current.play()}catch{return}start.current=performance.now()/1000-audio.current.currentTime}else start.current=performance.now()/1000;setRunning(true)};
