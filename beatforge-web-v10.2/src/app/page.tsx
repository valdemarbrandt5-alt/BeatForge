...
type SavedChart={id:string,title:string,difficulty:Difficulty,lane_count:number,duration:number,notes:Note[],created_at:string};
...
export default function Home(){
 ...
 const saveChart=async()=>{if(!user||!supabase){setAuthOpen(true);return}if(!(await ensureProfile()))return;const cleanNotes=baseNotes.map(({id,time,lane,duration})=>({id,time,lane,...(duration?{duration}:{})}));const {error}=await supabase.from('charts').insert({user_id:user.id,title:songName,difficulty,lane_count:laneCount,duration:songDuration,notes:cleanNotes});setCloudMessage(error?error.message:'Chart saved ✓');if(!error)setTimeout(()=>setCloudMessage(''),1800)};
 const loadMyCharts=async()=>{if(!user||!supabase){setAuthOpen(true);return}const {data,error}=await supabase.from('charts').select('id,title,difficulty,lane_count,duration,notes,created_at').eq('user_id',user.id).order('created_at',{ascending:false});if(error){setCloudMessage(error.message);return}setSavedCharts((data||[]) as SavedChart[]);setMyChartsOpen(true)};
 const openSavedChart=(chart:SavedChart)=>{const source=(chart.notes||[]).map((n,i)=>({...n,id:i,hit:false,miss:false,holding:false,completed:false}));if(audio.current){audio.current.pause();audio.current.currentTime=0}if(urlRef.current){URL.revokeObjectURL(urlRef.current);urlRef.current=null}setAudioUrl(null);fileRef.current=null;setSongName(chart.title);setDifficulty(chart.difficulty);setLaneCount(chart.lane_count);setDuration(chart.duration);setBaseNotes(source);setNotes(buildChart(source,chart.lane_count,chart.difficulty));setStatus('Saved chart loaded · upload the matching audio file to play with music');setMyChartsOpen(false)};
