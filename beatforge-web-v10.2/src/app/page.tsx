...
export default function Home(){
 const [laneCount,setLaneCount]=useState(5),[difficulty,setDifficulty]=useState<Difficulty>('Medium'),[baseNotes,setBaseNotes]=useState<Note[]>(demo),[keys,setKeys]=useState(defaults),[notes,setNotes]=useState<Note[]>(demo),[running,setRunning]=useState(false),[time,setTime]=useState(0),[combo,setCombo]=useState(0),[score,setScore]=useState(0),[judge,setJudge]=useState<Feedback>('READY'),[pressed,setPressed]=useState<number|null>(null),[impact,setImpact]=useState<{lane:number,id:number,kind:Feedback}|null>(null),[songName,setSongName]=useState('Demo chart'),[audioUrl,setAudioUrl]=useState<string|null>(null),[analyzing,setAnalyzing]=useState(false),[status,setStatus]=useState('Upload a song, or play the demo.'),[duration,setDuration]=useState(34),[volume,setVolume]=useState(0.8),[resultsOpen,setResultsOpen]=useState(false),[heldLanes,setHeldLanes]=useState<boolean[]>(Array(5).fill(false)),[hitStats,setHitStats]=useState({perfect:0,great:0,good:0,miss:0,maxCombo:0,timingSum:0,timingHits:0}),[holdScore,setHoldScore]=useState(0),[countIn,setCountIn]=useState<string|null>(null);
 const audio=useRef<HTMLAudioElement|null>(null),raf=useRef(0),start=useRef(0),timeRef=useRef(0),impactId=useRef(0),urlRef=useRef<string|null>(null),fileRef=useRef<File|null>(null),progressKnob=useRef<HTMLDivElement|null>(null),progressFill=useRef<HTMLDivElement|null>(null),remainingLabel=useRef<HTMLSpanElement|null>(null),heldLanesRef=useRef<boolean[]>(Array(5).fill(false)),comboRef=useRef(0),lastFrameRef=useRef<number|null>(null),holdRemainder=useRef(0);
 ...
 const play=async()=>{setResultsOpen(false);resetStats();setCombo(0);comboRef.current=0;setScore(0);setHoldScore(0);holdRemainder.current=0;lastFrameRef.current=null;setJudge('READY');setPressed(null);resetNotes();setTime(0);timeRef.current=0;setRunning(false);
  ...
  if(audio.current&&audioUrl){audio.current.currentTime=0;try{await audio.current.play()}catch{return}start.current=performance.now()/1000-audio.current.currentTime}else start.current=performance.now()/1000;lastFrameRef.current=null;setRunning(true)};
 useEffect(()=>{if(!running)return;let lastUiUpdate=0;const loop=()=>{const now=performance.now()/1000;const dt=lastFrameRef.current==null?0:Math.min(.05,Math.max(0,now-lastFrameRef.current));lastFrameRef.current=now;const t=audio.current&&audioUrl?audio.current.currentTime:now-start.current;timeRef.current=t;const total=audio.current&&audioUrl&&Number.isFinite(audio.current.duration)&&audio.current.duration>0?audio.current.duration:(audioUrl?duration:34);const smoothT=audio.current&&audioUrl&&!audio.current.paused?Math.min(total,t+Math.max(0,now-(start.current+t))):t;const p=total?Math.max(0,Math.min(1,smoothT/total)):0;if(progressFill.current)progressFill.current.style.height=`${p*100}%`;if(progressKnob.current)progressKnob.current.style.bottom=`calc(${p*100}% - 8px)`;if(remainingLabel.current)remainingLabel.current.textContent=fmt(Math.max(0,total-smoothT));timeRef.current=smoothT;setTime(smoothT);setNotes(old=>{let missedCount=0;let holdBonus=0;let changed=false;const next=old.map(n=>{if(!n.hit&&!n.miss&&smoothT-n.time>.16){missedCount++;changed=true;return{...n,miss:true}}if(n.holding&&!n.miss){const end=n.time+(n.duration||0);if(heldLanesRef.current[n.lane]&&smoothT<end){holdBonus+=dt*250*comboMultiplier(comboRef.current)}if(smoothT>=end){changed=true;return{...n,holding:false,completed:true}}}return n});if(holdBonus){holdRemainder.current+=holdBonus;const pts=Math.floor(holdRemainder.current);if(pts>0){holdRemainder.current-=pts;setScore(s=>s+pts);setHoldScore(s=>s+pts)}}if(missedCount){setCombo(0);comboRef.current=0;setJudge('MISS');setHitStats(s=>({...s,miss:s.miss+missedCount}))}return changed?next:old});if(audio.current&&audioUrl&&audio.current.ended)finishSong();else if(!audioUrl&&t>34)finishSong();else raf.current=requestAnimationFrame(loop)};raf.current=requestAnimationFrame(loop);return()=>cancelAnimationFrame(raf.current)},[running,audioUrl,duration]);
 ...
 const remaining=Math.max(0,songDuration-time);
 const multiplier=comboMultiplier(combo);
 const performancePoints=hitStats.perfect+hitStats.great*.8+hitStats.good*.5;
 const chartProgress=songDuration>0?Math.max(0,Math.min(1,time/songDuration)):0;
 ...
 const progress=songDuration?Math.min(1,time/songDuration):0;
